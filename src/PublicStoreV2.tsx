...
import { ArrowLeft, ArrowRight, Check, ChevronLeft, ChevronRight, Grid2X2, ImageOff, Minus, Plus, Search, ShoppingBag, Sparkles, X } from 'lucide-react'
import { api } from './api'
...
function ResilientImage({ src, alt, className = '', loading = 'lazy', size = 'medium' }: { src?: string; alt: string; className?: string; loading?: 'eager' | 'lazy'; size?: MediaVariantSize }) {
  const [failed, setFailed] = useState(false)
  useEffect(() => { setFailed(false) }, [src, size])
  if (!src || failed) return <div className={`media-fallback ${className}`} role="img" aria-label={alt}><ImageOff size={28}/><span>Imagem indisponível</span></div>
  return <img className={className} src={optimizedMediaUrl(src, size)} alt={alt} loading={loading} decoding="async" onError={() => setFailed(true)} />
}
...
function ProductMedia({ product, className = '', size = 'medium' }: { product: Product; className?: string; size?: MediaVariantSize }) {
  ...
  return <ResilientImage className={className} src={mediaUrl(product)} alt={product.name} size={size} />
}
...
export default function PublicStoreV2() {
  ...
  return <div className={`app ${view === 'feed' ? 'app--feed' : ''}`} style={{ '--accent': store?.accent || '#c94c2d' } as React.CSSProperties}>
    <header className="topbar">
      <button className="brand" onClick={() => setView('store')}>{store?.logoUrl ? <img className="brand__logo" src={optimizedMediaUrl(store.logoUrl, 'thumb')} alt="" /> : <span className="brand__mark">SV</span>}<span className="brand__text"><strong>{store?.name}</strong><small>via Shopvax</small></span></button>
    ...
    </main> : <main className="feed-shell"><div className="feed-list">{products.map((product) => <article className="feed-card" key={product.id}><ProductMedia product={product} size="large"/><div className="feed-card__shade"/><div className="feed-card__top"><span>{product.category}</span><span>{product.sku}</span></div><div className="feed-card__actions"><button onClick={() => openPicker(product)}><Plus size={24}/></button></div><div className="feed-card__info"><span className="feed-card__seller">com {seller?.name || 'atendimento'}</span><h2>{product.name}</h2><p>{product.description}</p><div className="feed-card__buyline"><strong>{money.format(product.price)}</strong><button onClick={() => openPicker(product)}>Escolher quantidade <ChevronRight size={18}/></button></div></div></article>)}{!demo && <div ref={loadMoreRef}/>}</div></main>}

    <div className={`drawer-backdrop ${cartOpen ? 'is-open' : ''}`} onClick={() => setCartOpen(false)}/>
    <aside className={`cart-drawer ${cartOpen ? 'is-open' : ''}`}><div className="cart-drawer__head"><div><small>Seu pedido</small><h2>Carrinho</h2></div><button onClick={() => setCartOpen(false)} aria-label="Fechar carrinho"><X size={22}/></button></div><div className="cart-drawer__seller"><span>Atendimento atribuído</span><strong>{seller?.name || 'Atendimento'}</strong></div><div className="cart-items">{cart.length === 0 ? <div className="cart-empty"><ShoppingBag size={30}/><h3>Seu carrinho está vazio.</h3></div> : cart.map((item) => <div className="cart-item" key={item.key}><ResilientImage src={mediaUrl(item.product)} alt={item.product.name} size="thumb"/><div className="cart-item__main"><span>{item.product.sku}</span><strong>{item.product.name}</strong><small>{Object.entries(item.selections).map(([k,v]) => `${k}: ${v}`).join(' · ')}</small><small>{money.format(item.product.price)} cada</small></div><div className="quantity-control"><button onClick={() => setCart((current) => current.map((x) => x.key === item.key ? {...x, quantity: Math.max(0, x.quantity - 1)} : x).filter((x) => x.quantity > 0))} aria-label={`Diminuir quantidade de ${item.product.name}`}><Minus size={14}/></button><b>{item.quantity}</b><button onClick={() => setCart((current) => current.map((x) => x.key === item.key ? {...x, quantity: x.quantity + 1} : x))} aria-label={`Aumentar quantidade de ${item.product.name}`}><Plus size={14}/></button></div></div>)}</div><div className="cart-drawer__foot"><div className="minimum-box"><div className="minimum-box__copy">{minimumReached ? <span className="minimum-ok"><Check size={15}/> Pedido mínimo atingido</span> : <span>Faltam {money.format(Math.max(0, minimum - cartTotal))}</span>}<strong>{money.format(cartTotal)} / {money.format(minimum)}</strong></div><div className="minimum-track"><span style={{width: `${minimumProgress}%`}}/></div></div><div className="cart-total"><span>Total dos produtos</span><strong>{money.format(cartTotal)}</strong></div>{error && <p className="form-error">{error}</p>}<button className="whatsapp-button" disabled={!minimumReached || !cart.length || sending} onClick={sendOrder}>{sending ? 'Preparando pedido…' : `Enviar pedido para ${seller?.name || 'atendimento'}`}<ArrowRight size={19}/></button></div></aside>

    {picker && <div className="picker-layer" role="dialog" aria-modal="true" aria-label={`Escolher ${picker.name}`}><button className="picker-layer__backdrop" onClick={() => setPicker(null)} aria-label="Fechar produto"/><section className="product-picker"><button className="product-picker__close" onClick={() => setPicker(null)} aria-label="Fechar"><X size={20}/></button><div className="product-picker__media product-gallery"><div className="product-gallery__stage">{picker.mediaType === 'video' && !pickerGallery.length ? <video src={picker.mediaUrl} controls playsInline/> : <ResilientImage src={pickerGallery[galleryIndex] || mediaUrl(picker)} alt={picker.name} size="large"/>} {pickerGallery.length > 1 && <><button className="product-gallery__prev" onClick={() => setGalleryIndex((i) => (i - 1 + pickerGallery.length) % pickerGallery.length)} aria-label="Foto anterior"><ChevronLeft size={20}/></button><button className="product-gallery__next" onClick={() => setGalleryIndex((i) => (i + 1) % pickerGallery.length)} aria-label="Próxima foto"><ChevronRight size={20}/></button></>}</div>{pickerGallery.length > 1 && <div className="product-gallery__thumbs">{pickerGallery.map((url, index) => <button key={url} className={index === galleryIndex ? 'is-active' : ''} onClick={() => setGalleryIndex(index)} aria-label={`Ver foto ${index + 1}`}><ResilientImage src={url} alt={`${picker.name} - foto ${index + 1}`} size="thumb"/></button>)}</div>}</div><div className="product-picker__body"><div className="product-picker__meta"><span>{picker.sku}</span><span>{picker.category}</span></div><h2>{picker.name}</h2><p>{picker.description}</p><strong className="product-picker__price">{money.format(picker.price)}</strong>{picker.variations.map((group) => <div className="variant-group" key={group.name}><div><strong>{group.name}</strong><span>{pickerSelections[group.name] || 'Escolha uma opção'}</span></div><div className="variant-options">{group.options.map((option) => <button key={option} className={pickerSelections[group.name] === option ? 'is-active' : ''} onClick={() => { setPickerSelections((current) => ({...current, [group.name]: option})); setError('') }}>{option}</button>)}</div></div>)}<div className="picker-quantity"><div><strong>Quantidade</strong><span>{money.format(picker.price * pickerQty)}</span></div><div className="quantity-control quantity-control--large"><button onClick={() => setPickerQty((v) => Math.max(1, v - 1))} aria-label="Diminuir quantidade"><Minus size={16}/></button><b>{pickerQty}</b><button onClick={() => setPickerQty((v) => v + 1)} aria-label="Aumentar quantidade"><Plus size={16}/></button></div></div>{error && <p className="form-error">{error}</p>}<button className="picker-add" onClick={confirmPicker}><ShoppingBag size={18}/> Adicionar {pickerQty} ao carrinho <span>{money.format(picker.price * pickerQty)}</span></button></div></section></div>}
