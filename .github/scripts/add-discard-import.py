...
# make running collector tolerate deletion: guard each persist chunk before writing
...
# insert button into job actions before status span
...
// completed: discarding history must not delete published catalog product
...
anchor="      - name: Scanner parent product integration\n        run: node server/scanner-parent-product.integration.mjs\n"
if 'Scanner discard import integration' not in s:
    if anchor not in s: raise SystemExit('ci anchor missing')
    s=s.replace(anchor,anchor+"      - name: Scanner discard import integration\n        run: node server/scanner-discard.integration.mjs\n")
p.write_text(s)
