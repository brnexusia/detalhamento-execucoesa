...
async function register() {
  const response = await fetch(`${base}/api/auth/register`, {
    ...
    body: JSON.stringify({ name: 'Teste Métricas', email: `metrics-${Date.now()}-${Math.random()}@example.test`, password: 'scanner1234', storeName: `Loja Métricas ${Date.now()}`, whatsapp: '5511999999999', planCode: 'bronze' }),
  })
