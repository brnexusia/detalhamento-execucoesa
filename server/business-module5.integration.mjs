...
async function register() {
  const response = await fetch(`${base}/api/auth/register`, {
    ...
    body: JSON.stringify({ name: 'Admin Equipe', email: `team-${Date.now()}-${Math.random()}@example.test`, password: 'scanner1234', storeName: `Loja Equipe ${Date.now()}`, whatsapp: '5511999999999', planCode: 'bronze' }),
  })
