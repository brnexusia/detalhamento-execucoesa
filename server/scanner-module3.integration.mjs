...
assert.equal(normalized.warning_count, 0, 'produto válido não deve exigir revisão só por faltar dado opcional')
assert.equal(normalized.selected_count, 2, 'todos os produtos publicáveis devem seguir pré-selecionados')
...
assert.deepEqual(bolsa.warnings, [], 'sem imagem/SKU/categoria explícita não deve bloquear produto com nome e preço')
assert.ok(bolsa.confidence < 1, 'completude menor continua registrada pela confiança')
