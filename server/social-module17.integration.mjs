...
if (!feedSource.includes('profilePath(post.store.slug, post.product.id)')) throw new Error('Compartilhamento perdeu o deep link social do produto.')
...
try {
  await pool.query('INSERT INTO users (id,email,name,password_hash) VALUES ($1,$2,$3,$4)', [userId, `m17-${suffix}@example.test`, 'Meta Teste', 'salt:hash'])
  await pool.query(`INSERT INTO stores (id,owner_id,slug,name,tagline,logo_url,whatsapp) VALUES ($1,$2,$3,$4,$5,$6,$7)`, [storeId, userId, slug, 'Loja & Meta', 'Catálogo especial & atacado.', 'https://cdn.example.test/logo.jpg', '5511999999999'])
  await pool.query(`INSERT INTO products (id,store_id,sku,name,description,price,category,media_url,media_type,active) VALUES ($1,$2,$3,$4,$5,$6,$7,$8,'image',true)`, [productId, storeId, 'META-1', 'Bolsa <Premium>', 'Produto "especial" & pronto para pedido.', 129.9, 'Bolsas', 'https://cdn.example.test/produto.jpg'])

  const productResponse = await fetch(`${baseUrl}/perfil/${slug}?produto=${encodeURIComponent(productId)}`)
  ...
  if (!productHtml.includes(`property="og:url" content="${baseUrl}/perfil/${slug}?produto=${productId}"`)) throw new Error('OG URL não preserva o deep link social do produto.')

  const profileResponse = await fetch(`${baseUrl}/perfil/${slug}`)
  const profileHtml = await profileResponse.text()
  if (!profileHtml.includes('<title>Loja &amp; Meta | Shopvax</title>')) throw new Error('Título do perfil da loja não foi injetado.')
  if (!profileHtml.includes('Catálogo especial &amp; atacado.')) throw new Error('Descrição do perfil não foi injetada.')
